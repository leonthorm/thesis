...
from util.load_dbcbs_traj import get_dbcbs_trajectory
...
class DbCbsEnv(MujocoEnv):
    ...
    def __init__(
        ...
        observation_space_shape = (n_robots, 9 + 3 * (n_robots - 1))

        observation_space = Box(low=-np.inf, high=np.inf, shape=observation_space_shape, dtype=np.float64)
        self.dagger = dagger
        MujocoEnv.__init__(
            self,
            xml_file,
        ...
        self.ts, self.pos_d, self.vel_d, self.acc_d = get_dbcbs_trajectory(traj_file, n_robots, self.dt)
    ...
    def set_traj(self, traj_file):
        ...
        self.ts, self.pos_d, self.vel_d, self.acc_d = get_dbcbs_trajectory(traj_file, self.n_robots, self.dt)
        self.reset_model()
        self.set_traj_name(traj_file)
