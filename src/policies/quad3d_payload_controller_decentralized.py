...
class Quad3dPayloadController:
    ...
    def __updateDesState(self, payload_pos, payload_vel,
                         quat, pos, w, vel,
                         other_robot_pos,
                         payload_pos_d, payload_vel_d,
                         quat_d, pos_d, w_d, vel_d,
                         actions_d, compAcc, a_ref, tick):
        self.setpoint.position.x = payload_pos_d[0]  # m
        self.setpoint.position.y = payload_pos_d[1]  # m
        self.setpoint.position.z = payload_pos_d[2]  # m
        ap = a_ref
        if self.payloadType == "point":
            start_idx = 0
            rig_idx = 0
        ...
        self.setpoint.velocity.x = payload_vel_d[0]  # m/s
        self.setpoint.velocity.y = payload_vel_d[1]  # m/s
        self.setpoint.velocity.z = payload_vel_d[2]  # m/s
        self.setpoint.acceleration.x = ap[0]  # m/s^2 update this to be computed from model
        self.setpoint.acceleration.y = ap[1]  # m/s^2 update this to be computed from model
        self.setpoint.acceleration.z = ap[2] # m/s^2 update this to be computed from model
        ...
        self.posp_e = payload_pos_d - payload_pos
        self.velp_e = payload_vel_d - payload_vel
        accp = ap + grav
        self.F_ref = self.mp * (
    ...
    def controllerLeePayload(
            self, actions_d, states_d, state, obs, tick, compAcc, a_ref
    ):
        (
            ...
            actions_d,
            my_id
        ) = deconstruct_obs(obs, self.num_robots)
        self.team_ids.remove(my_id)
        self.team_ids.insert(0, my_id)
        self.__updateDesState(payload_pos, payload_vel,
                              quat, pos, w, vel,
                              other_robot_pos,
                              payload_pos_d, payload_vel_d,
                              quat_d, pos_d, w_d, vel_d,
